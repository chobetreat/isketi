...
if intro:
    ...
    print()
    sleep(1)
    prin(f"Currently Loading:   < {workingdir} >   Level Pack.")
    sleep(0.5)
    print()
    prin("Press enter to continue...")
    prin("Or, to load a different level pack, Type the name here:")
    i=input("load:  ")
    print("")
    startNoise=pygame.mixer.Sound("audio/start.wav")
    startNoise.set_volume(0.05)
    startNoise.play()
    for i in range(100):

        progressbar("Loading:", i, "complete...")
        sleep(random.randint(5, 10)/1000)
...
class connectedObject:
    def __init__(self, points, radius, color, connections, elasticity=0.5, polygon=[]):
        ...
        self.end=False
        for point in points:
            self.balls.append(ball(point[0], point[1], radius, color))
...
if not "draw_rects" in sket:
    GAME.draw_rects = False
else:
    GAME.draw_rects=sket["draw_rects"]
if not "draw_holes" in sket:
    GAME.draw_holes = False
else:
    GAME.draw_holes=sket["draw_holes"]
if not "draw_fans" in sket:
    GAME.draw_fans = False
else:
    GAME.draw_fans=sket["draw_fans"]
